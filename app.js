...
let state={matches:[],training:[],opponents:[],matchDefaults:{myWeight:"",myHeight:"",oppWeight:"",oppHeight:""}};
...
if(!Array.isArray(state.opponents))state.opponents=[];if(!state.matchDefaults||typeof state.matchDefaults!=="object")state.matchDefaults={myWeight:"",myHeight:"",oppWeight:"",oppHeight:""};["myWeight","myHeight","oppWeight","oppHeight"].forEach(k=>{if(typeof state.matchDefaults[k]!=="string")state.matchDefaults[k]=""});
...
function setNewMatchDate(){ if($("matchDate")&&!editingMatchId) $("matchDate").value=todayLocal(); }
function loadMatchDefaults(){
 const d=state.matchDefaults||{};
 $("myWeight").value=d.myWeight||"";
 $("myHeight").value=d.myHeight||"";
 $("oppWeight").value=d.oppWeight||"";
 $("oppHeight").value=d.oppHeight||"";
}
function saveMatchDefaults(){
 state.matchDefaults={myWeight:$("myWeight").value||"",myHeight:$("myHeight").value||"",oppWeight:$("oppWeight").value||"",oppHeight:$("oppHeight").value||""};
 save();
}
const form={type:"Practice",arm:"Right",singleStrap:"No",side:"Left",result:"Win",go:"I hit first",myTech:"Not recorded",oppTech:"Not recorded",length:"Not recorded",trainingArm:"Both",quality:"Good",superFormat:"Best of 3",superResult:"Win"};
let scope="Tournament",armScope="All",roundNumber=0,editingMatchId=null,editingTrainingId=null;
...
function resetMatchForm(){editingMatchId=null;$("matchForm").reset();loadMatchDefaults();$("matchDate").value=todayLocal();resetRoundEditor();setPenaltyValue("matchFouls",0);setPenaltyValue("matchWarnings",0);form.type="Practice";form.arm="Right";form.singleStrap="No";form.side="Left";form.result="Win";form.go="I hit first";$("matchMode").textContent="New Match";$("cancelMatchEdit").classList.add("hidden");setGroup("matchType","type","Practice");setGroup("arm","arm","Right");setGroup("singleStrap","singleStrap","No");setGroup("tableSide","side","Left");setGroup("result","result","Win");setGroup("go","go","I hit first");setGroup("myTech","myTech","Not recorded");setGroup("oppTech","oppTech","Not recorded");setGroup("length","length","Not recorded");form.superFormat="Best of 3";form.superResult="Win";setGroup("superFormat","superFormat","Best of 3");setGroup("superResult","superResult","Win");$("singleFields").classList.remove("hidden");$("superFields").classList.add("hidden")}
function loadMatchForEdit(id){const m=state.matches.find(x=>x.id===id);if(!m)return;editingMatchId=id;page("match");$("matchMode").textContent="Edit Match";$("cancelMatchEdit").classList.remove("hidden");$("opponent").value=m.opponent||"";$("matchDate").value=m.date||todayLocal();$("myWeight").value=m.myWeight||"";$("oppWeight").value=m.oppWeight||"";$("myHeight").value=m.myHeight||"";$("oppHeight").value=m.oppHeight||"";$("weightClass").value=m.weightClass||"Not recorded";$("comments").value=m.comments||"";setPenaltyValue("matchFouls",m.fouls);setPenaltyValue("matchWarnings",m.warnings);setGroup("matchType","type",m.matchType);setGroup("arm","arm",m.arm);setGroup("tableSide","side",m.side);if(m.matchType==="Supermatch"){$("singleFields").classList.add("hidden");$("superFields").classList.remove("hidden");setGroup("superFormat","superFormat",m.superFormat||"Best of 3");setGroup("superResult","superResult",m.superResult||m.result||"Win");resetRoundEditor();(m.rounds||[]).forEach(r=>addRound(r))}else{$("singleFields").classList.remove("hidden");$("superFields").classList.add("hidden");setGroup("singleStrap","singleStrap",m.strap||"No");setGroup("result","result",m.result||"Win");setGroup("go","go",m.go||"Unclear");setGroup("myTech","myTech",m.myTech||"Not recorded");setGroup("oppTech","oppTech",m.oppTech||"Not recorded");setGroup("length","length",m.length||"Not recorded")}}
...
function colorizeResults(){
 document.querySelectorAll("body *").forEach(el=>{
  ...
  if(t==="win"||t==="i won"){
    ...
    el.dataset.result="Win";
  }else if(t==="loss"||t==="i lost"){
    el.classList.add("loss");
    el.dataset.result="Loss";
  }
 });
}
...
colorizeResults();
const resultColorObserver=new MutationObserver(()=>colorizeResults());
resultColorObserver.observe(document.body,{subtree:true,childList:true});
...
for(let i=1;i<=10;i++)$("sets").insertAdjacentHTML("beforeend",`<option>${i}</option>`);["myWeight","myHeight","oppWeight","oppHeight"].forEach(id=>$(id)?.addEventListener("change",saveMatchDefaults));
...
function bindRoundChoices(container,selector,getValue,setValue){
 container.querySelectorAll(selector).forEach(b=>{
  ...
  b.addEventListener("click",()=>{
   setValue(b.dataset.value);
   container.querySelectorAll(selector).forEach(x=>{
    const on=x===b;
    x.classList.toggle("selected",on);
    x.setAttribute("aria-pressed",on?"true":"false");
   });
...
function dashboardScopedMatches(){
 let matches=scope==="All"?state.matches:state.matches.filter(m=>m.matchType===scope);
 if(armScope!=="All")matches=matches.filter(m=>m.arm===armScope);
 return matches;
}
function renderTechniqueBreakdown(){
  const matches=dashboardScopedMatches();
...
function renderDashboard(){
 let t=rec(state.matches.filter(m=>m.matchType==="Tournament")),
     s=rec(state.matches.filter(m=>m.matchType==="Supermatch")),
     p=rec(state.matches.filter(m=>m.matchType==="Practice"));
 $("tourRecord").textContent=t.join("–");
 $("superRecord").textContent=s.join("–");
 $("practiceRecord").textContent=p.join("–");
 $("trainingCount").textContent=state.training.length;

 let ms=dashboardScopedMatches(),st=techniqueStats(ms),
     all=Object.entries(st.mine).map(([k,v])=>({k,...v,n:v.w+v.l})),
     fav=all.sort((a,b)=>b.n-a.n)[0],
     good=all.filter(x=>x.n>=3).sort((a,b)=>b.w/b.n-a.w/a.n)[0],
     bad=all.filter(x=>x.n>=3).sort((a,b)=>a.w/a.n-b.w/b.n)[0],
     opp=Object.entries(st.against).map(([k,v])=>({k,...v,n:v.w+v.l}))
       .filter(x=>x.n>=3).sort((a,b)=>b.w/b.n-a.w/a.n)[0];

 $("myStats").innerHTML=`<div class="two">
  <div class="rec"><b>Favorite technique</b><br>${fav?esc(fav.k):"Not enough data"}</div>
  <div class="rec"><b>Most successful</b><br>${good?esc(good.k)+" · "+Math.round(good.w/good.n*100)+"% win rate":"Need 3+ attempts"}</div>
  <div class="rec"><b>Least successful</b><br>${bad?esc(bad.k)+" · "+Math.round(bad.w/bad.n*100)+"% win rate":"Need 3+ attempts"}</div>
  <div class="rec"><b>Best opponent technique against you</b><br>${opp?esc(opp.k)+" · opponents win "+Math.round(opp.w/opp.n*100)+"%":"Need 3+ attempts"}</div>
 </div>`;
 $("recent").innerHTML=state.matches.slice(0,5).map(m=>`<div class="entry"><div class="row"><b>${esc(m.opponent)}</b><b class="${m.result==="Win"?"win":"loss"}">${m.result.toUpperCase()}</b></div><div class="muted">${esc(m.matchType)} · ${esc(m.arm)} arm${m.matchType==="Supermatch"?" · "+(m.rounds||[]).length+" rounds":""}</div></div>`).join("")||'<p class="muted">No matches yet.</p>';
 renderTechniqueAnalytics();
 renderTechniqueBreakdown();
}
...
bindReadyGo();
bindVoice();
if("serviceWorker" in navigator)navigator.serviceWorker.register("sw.js?v=950").catch(()=>{});
})();
