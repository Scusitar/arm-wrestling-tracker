...
let rgVoice=null;
function rgInitVoice(){try{if(!("speechSynthesis" in window))return false;const voices=window.speechSynthesis.getVoices();rgVoice=voices.find(v=>/en[-_]CA/i.test(v.lang))||voices.find(v=>/en[-_]US/i.test(v.lang))||voices.find(v=>/^en/i.test(v.lang))||null;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance("Ready");u.voice=rgVoice;u.lang=rgVoice?.lang||"en-CA";u.rate=.92;u.pitch=.98;u.volume=1;window.speechSynthesis.speak(u);window.speechSynthesis.cancel();return true}catch(e){return false}}
